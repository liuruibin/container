...
The images are intended for use in the **FROM** field of a downstream Dockerfile. For example, specify `FROM ghcr.io/graalvm/graalvm-community:latest` or a version tag.

# Building the images

For building GraalVM Java 25 amd64 and arm64 images use:

```
docker buildx build --platform linux/amd64 --build-arg GRAALVM_VERSION=<GraalVM Version> --build-arg JAVA_VERSION=java25 -t ghcr.io/graalvm/graalvm-community:<Java Version>-<OS Version>-amd64 --output=type=docker -f Dockerfile.<OL Version>-java25 .
```

```
docker buildx build --platform linux/arm64 --build-arg GRAALVM_VERSION=<GraalVM Version> --build-arg JAVA_VERSION=java25 -t ghcr.io/graalvm/graalvm-community:<Java Version>-<OS Version>-arm64 --output=type=docker -f Dockerfile.<OL Version>-java25 .
```
...
The GraalVM CE Docker image uses [Oracle Linux](https://hub.docker.com/_/oraclelinux/) as its base image. See [Oracle Linux End-User License Agreement](https://oss.oracle.com/ol9/EULA).
